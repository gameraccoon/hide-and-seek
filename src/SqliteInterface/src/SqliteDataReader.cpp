...
SqliteDataReader::SqliteDataReader(const std::string& query, sqlite3* db)
{
	...
	if (rc != 0)
	{
		LogError(std::string("Unable to execute SQL query \"").append(query)
...
DbValue::Ptr SqliteDataReader::getValueByName(const std::string& columnName)
{
	...
	LogError("SqliteDataReader error: Column not found: " + columnName);
	return nullptr;
}
...
DbValue::Ptr SqliteDataReader::getValueByIndex(int columnIndex)
{
	...
	{
		LogError("SqliteDataReader error: Column not found: " + std::to_string(columnIndex));
	}
