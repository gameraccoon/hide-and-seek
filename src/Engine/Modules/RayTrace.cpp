...
namespace Raytrace
{
	...
	inline bool checkIntersect2Lines(Vector2D A1, Vector2D A2, Vector2D B1, Vector2D B2)
	{
	...
	inline Vector2D getPointIntersect2Lines(Vector2D A1, Vector2D A2, Vector2D B1, Vector2D B2)
	{
		float DA1 = A1.y - A2.y;
		float DB1 = A2.x - A1.x;
...
bool RayTrace::fastTrace()
{
	for (auto const &currentActor : this->ownerWorld->allActors)
	{
		...
		int a = Raytrace::getDotCode(&box, &this->startPoint);
		// get cohen's code for end point
		int b = Raytrace::getDotCode(&box, &this->endPoint);
		...
		if (((a == 0 || b == 0)	&& (a | b) != 0)	// one point is in BB another is out BB
			...
			||
			Raytrace::checkIntersectAABBLine(&box, &this->startPoint, &this->endPoint))	// other cases of intersection
		{
			//return true;
	...
	for (auto const &currentActor : this->ownerWorld->allActors)
	{
			...
			Raytrace::checkIntersectAABBLine(&box, &this->startPoint, &this->endPoint))	// other cases of intersection
		{
			...
			for (auto &border : hull->borders)
			{
				...
				// if raytrace intersect this border
				if (!Raytrace::checkIntersect2Lines(actorsLocation + border.getA(), actorsLocation + border.getB(), this->startPoint, this->endPoint))
					continue;
